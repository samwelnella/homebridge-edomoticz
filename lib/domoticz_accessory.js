...
function eDomoticzAccessory(platform, platformAccessory, IsScene, status, idx, name, uuid, haveDimmer, maxDimLevel, subType, Type, batteryRef, swType, swTypeVal, hwType, image, eve, hwtimeout) {
    ...
    handleMQTTMessage: function (message, callback) {
        ...
        if (this.isSwitch) {
            ...
            case this.swTypeVal == Constants.DeviceTypeContact:
                {
                    if (this.image !== undefined && this.image.indexOf("Water" > -1) {
                        var service = this.getService(Service.LeakSensor);
                        var characteristic = this.getCharacteristic(service, Characteristic.LeakDetected);
                        callback(characteristic, message.nvalue);
                    } else {
                    }
                }
            ...
            case this.Type == "General" || this.Type == "YouLess Meter" || this.Type == "Current" || this.Type == "UV" || this.Type == "Usage" || this.Type == "Lux":
                {
                    if ('undefined' !== typeof message.svalue1) {
                        if (this.subType == "kWh" || this.subType == "YouLess counter" || this.subType == "Electric") {
                            ...
                            var UsageDeviceCharacteristic = this.getCharacteristic(service, eDomoticzServices.Barometer);
            ...
            case this.Type == "Temp + Humidity + Baro":
                {
                    if ('undefined' !== typeof message.svalue1) {
                        ...
                        var service2 = this.getService(Service.TemperatureSensor);
                        var temperatureSensorCharacteristic2 = this.getCharacteristic(service2, Characteristic.CurrentTemperature);
                        callback(temperatureSensorCharacteristic2, temperature);
                    }
                    if ('undefined' !== typeof message.svalue2) {
                        var service = this.getService(eDomoticzServices.WeatherService);
                        var HumidityCharacteristic = this.getCharacteristic(service, Characteristic.CurrentRelativeHumidity);
                        ...
                        if (!service2) {
                            service2 = this.getService(Service.TemperatureSensor);
                        }
                        var HumidityCharacteristic2 = this.getCharacteristic(service2, Characteristic.CurrentRelativeHumidity);
                        callback(HumidityCharacteristic2, humidity);
                    }
    ...
    getServices: function () {
        ...
        if (this.isSwitch) {
            ...
            case this.swTypeVal == Constants.DeviceTypeContact:
                {
                    if (this.image !== undefined && this.image.indexOf("Water" > -1) {
                        var leakService = this.getService(Service.LeakSensor);
                         if (!leakService) {
                             leakService = new Service.LeakSensor(this.name);
                         }
                         this.getCharacteristic(leakService, Characteristic.LeakDetected).on('get', this.getStringValue.bind(this));
                         if (this.batteryRef && this.batteryRef !== 255) { // if batteryRef == 255 we're running on mains
                             this.gracefullyAddCharacteristic(leakService, Characteristic.StatusLowBattery).on('get', this.getLowBatteryStatus.bind(this));
                         }
                         this.services.push(leakService);
                        } else {
                        }
            ...
            case this.swTypeVal == Constants.DeviceTypeDimmer:
                {
                    ...
                    if (isFan) {
                        ...
                        if (this.subType == "RGBW" || this.subType == "RGBWW" || this.subType == "RGBWWZ" || this.subType == "RGB" ) {
            ...
            case this.Type == "General" || this.Type == "RFXMeter" || this.Type == "YouLess Meter" || this.Type == "Current" || this.Type == "UV" || this.Type == "Usage" || this.Type == "Lux":
                {
                    if (this.subType == "kWh" || this.subType == "YouLess counter" || this.subType == "Electric") {
                        ...
                        var Baroservice = this.getService(eDomoticzServices.WeatherService);
                        if (!Baroservice) {
                            Baroservice = new eDomoticzServices.WeatherService(this.name);
                        }
                        this.getCharacteristic(Baroservice, eDomoticzServices.Barometer).on('get', this.getPressure.bind(this));
                        this.services.push(Baroservice);
                        break;
                    } else if (this.subType == "Visibility") {
            ...
            case this.Type == "Temp" || this.Type == "Temp + Humidity" || this.Type == "Temp + Humidity + Baro":
                {
                    ...
                    this.gracefullyAddCharacteristic(temperatureSensorService, Characteristic.StatusFault).on('get', this.getTimeout.bind(this));
                    this.services.push(temperatureSensorService);

                    if (this.Type == "Temp + Humidity + Baro") {
                        var additionalSensorService = this.getService(Service.TemperatureSensor);
                        if (!additionalSensorService) {
                            additionalSensorService = new Service.TemperatureSensor(this.name);
                        }
                        this.getCharacteristic(additionalSensorService, Characteristic.CurrentTemperature).on('get', this.getTemperature.bind(this));
                        this.getCharacteristic(additionalSensorService, Characteristic.CurrentTemperature).setProps({
                            minValue: -50
                        });
                        this.gracefullyAddCharacteristic(additionalSensorService, Characteristic.CurrentRelativeHumidity).on('get', this.getHumidity.bind(this));
